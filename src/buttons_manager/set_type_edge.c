...
void    set_type_edge(t_viewer *viewer, GtkWidget *box_buttons)
{
    ...
    g_signal_connect(GTK_CHECK_BUTTON(button_solid), "toggled", G_CALLBACK(select_solid_type), viewer);
    button_dashed = gtk_check_button_new_with_label("Dashed");
    gtk_check_button_set_group (GTK_CHECK_BUTTON(button_dashed), GTK_CHECK_BUTTON(button_solid));
    gtk_box_append(GTK_BOX(box_proj), button_dashed);
    g_signal_connect(GTK_CHECK_BUTTON(button_dashed), "toggled", G_CALLBACK(select_dashed_type), viewer);
}