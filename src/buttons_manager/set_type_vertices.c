...
void set_type_vertices(t_viewer *viewer, GtkWidget *box_buttons) {
  ...
  g_signal_connect(GTK_CHECK_BUTTON(button_none), "toggled",
                   G_CALLBACK(select_none_vertices), viewer);
  button_circle = gtk_check_button_new_with_label("Circle");
  gtk_check_button_set_group(GTK_CHECK_BUTTON(button_none),
                             GTK_CHECK_BUTTON(button_circle));
  gtk_box_append(GTK_BOX(box_types), button_circle);
  g_signal_connect(GTK_CHECK_BUTTON(button_circle), "toggled",
                   G_CALLBACK(select_circle_vertices), viewer);
  button_square = gtk_check_button_new_with_label("Square");
  gtk_check_button_set_group(GTK_CHECK_BUTTON(button_none),
                             GTK_CHECK_BUTTON(button_square));
  gtk_check_button_set_group(GTK_CHECK_BUTTON(button_circle),
                             GTK_CHECK_BUTTON(button_square));
  gtk_box_append(GTK_BOX(box_types), button_square);
  g_signal_connect(GTK_CHECK_BUTTON(button_square), "toggled",
                   G_CALLBACK(select_square_vertices), viewer);
  if (viewer->settings.vertices_type == 0)
    gtk_check_button_set_active(GTK_CHECK_BUTTON(button_none), TRUE);
  else if (viewer->settings.vertices_type == 1)
    gtk_check_button_set_active(GTK_CHECK_BUTTON(button_circle), TRUE);
  else
    gtk_check_button_set_active(GTK_CHECK_BUTTON(button_square), TRUE);
}