...
static void rotate_pos(GtkButton *btn, t_viewer *viewer)
{
    ...
    buf = gtk_entry_get_buffer(GTK_ENTRY(viewer->entry.entry_rotate_x));
...
void input_rotating_x(t_viewer *viewer, GtkWidget *box_rotating) {
    ...
    gtk_entry_set_placeholder_text(GTK_ENTRY(viewer->entry.entry_rotate_x), "input degree for rotate x");
