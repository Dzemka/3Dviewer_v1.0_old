...
static void change_edge_color(GtkWidget *btn, t_viewer *viewer)
{
    GdkRGBA color;

    gtk_color_chooser_get_rgba(GTK_COLOR_CHOOSER(btn), &viewer->info.edge_color);
...
void set_color_edge(t_viewer *viewer, GtkWidget *box_buttons)
{
    ...
    gtk_color_button_set_title(GTK_COLOR_BUTTON(color_button), "Select edge color");
    gtk_color_button_set_modal(GTK_COLOR_BUTTON(color_button), TRUE);
    g_signal_connect_after(GTK_COLOR_BUTTON(color_button), "color-set", G_CALLBACK(change_edge_color), viewer);
    frame = gtk_frame_new("Select edge color");
