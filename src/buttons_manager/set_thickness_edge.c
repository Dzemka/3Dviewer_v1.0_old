...
{
    viewer->info.edge_thickness = gtk_range_get_value(GTK_RANGE(btn));
...
void set_thickness_edge(t_viewer *viewer, GtkWidget *box_buttons)
{
    ...
    thickness_scale = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 1, 5, 1);
    gtk_frame_set_child(GTK_FRAME(frame), thickness_scale);
    g_signal_connect_after(GTK_SCALE(thickness_scale), "value-changed", G_CALLBACK(set_thickness), viewer);
}