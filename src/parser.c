...
void parser(char *s, t_viewer *viewer) {
  ...
  while ((read = getline(&line, &len, file)) != -1) {
    if ((line[0] == 'v' || line[0] == 'f') && line[1] == ' ') {
      ...
      // можно улучшить логику и разрезать строку на слова только если после
      // пробега всех пробелов символ равен 'v' или 'f', а следущий символ -
      // пробел (пробельный)
      if (strcmp(tmp[0], "v") == 0) {
        if (i < 4) {
        ...
        while (++i < 3)
            p[i] = atof(tmp[i + 1]);
        t_list *tmp_point = ft_lstnew((void *)(p));
        if (tmp_point == NULL) {
  ...
  free(line);
  viewer->p = malloc((sizeof(double *) * 3 * count_point));
  i = -1;
  count_point *= 3;
  t_list *tmp_list;
  ...
  i = 0;
  while (tmp_list) {
    viewer->p[i] = ((double *)tmp_list->content)[0];
    viewer->p[i + 1] = ((double *)tmp_list->content)[1];
    viewer->p[i + 2] = ((double *)tmp_list->content)[2];
    tmp_list = tmp_list->next;
    i += 3;
  }
  viewer->count_point = count_point;
  ...
  i = -1;
//  while (++i < 100)
//    printf("%f %f %f %f\n", viewer->p[0], viewer->p[1], viewer->p[2]);
//  exit(1);
  return;
}
...
size_t ft_strlcpy(char *dst, const char *src, size_t dstsize) {
  ...
  if (src == NULL) return ((size_t)NULL);
  i = 0;
  res = 0;
  while (src[res]) res++;
  if (dstsize < 1) return (res);
...
static unsigned int ft_get_nb_strs(char const *s, const char c) {
  ...
  while (s[i]) {
    if (split_compare(c, s[i]))
      if (i > 0 && !split_compare(c, s[i - 1])) str_count++;
    i++;
  }
  if (!split_compare(c, s[i - 1])) str_count++;
  return (str_count);
}
...
static void ft_get_next_str(char **next_str, unsigned int *next_str_len,
  ...
  *next_str_len = 0;
  i = 0;
  while (**next_str && split_compare(c, **next_str)) (*next_str)++;
  while ((*next_str)[i]) {
    if (split_compare(c, (*next_str)[i])) return;
    (*next_str_len)++;
    i++;
  }
}
...
char **ft_split(char const *s, const char c) {
  ...
  while (i < str_nbr) {
    ...
    if (!split[i]) ft_clear_splitted(split);
